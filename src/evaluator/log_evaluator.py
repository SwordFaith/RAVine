''' Implementation of the agentic research evaluation module (from logs) '''
...
from src.agent import AgenticMessageHistory
...
class LogEvaluator:
    ...
    def __init__(self,
        ...
        with open(os.path.join(self.run_log_dir, 'eval.jsonl'), 'r') as file:
            self.evals = [json.loads(line) for line in file]
        
        with open(os.path.join(self.run_log_dir, 'final_eval.json'), 'r') as file:
            self.final_eval = json.load(file)
    ...
    def run_eval(self):
        ...
        for topic in tqdm(self.topics):
            ...
            ### load the running log
            log_filename = os.path.join(self.run_log_dir, f'{topic.qid}.log')
            with open(log_filename, 'r') as file:
                content = file.read()
                data = json.loads(content)
                messages = data['_messages']
            history = AgenticMessageHistory(messages=messages)
            eval_result = [item for item in self.evals if item.get('qid') == topic.qid][0]


            ### system-level efficiency assessment
            rm.latency = eval_result['latency']
            rm.cost = eval_result['cost']
            rm.new_tokens_count = eval_result['new_tokens_count']
            rm.iteration_turns = eval_result['iteration_turns']
            ...
            if eval_result['is_failed']:
                rm.is_failed = True
                mm.num_failed_runs += 1.0
                continue
            else:
                rm.report = eval_result['report']
            
            rm.blocks = eval_result['blocks']
            rm.block_nuggets_assignment = eval_result['block_nuggets_assignment']
            rm.global_nuggets_assignment = eval_result['global_nuggets_assignment']

            num_cited_blocks = 0.
            for idx, (block_text, citations) in enumerate(rm.blocks):
                assigned_nuggets = [AssignedScoredNugget(**item) for item in rm.block_nuggets_assignment[idx]]
                ...
                for i, nugget in enumerate(assigned_nuggets):
                    if nugget.importance == 'vital':
                        if nugget.assignment == 'support' or nugget.assignment == 'partial_support':
        ...
        mm.avg_task_completeness = (mm.avg_task_completeness / mm.num_valid_run) if mm.num_valid_run > 0 else 0.0
        mm.final_task_completeness = mm.avg_task_completeness * mm.task_finish_rate # consider failed runs in calculation
        mm.avg_citation_recall = (mm.avg_citation_recall / mm.num_valid_run) if mm.num_valid_run > 0 else 0.0
        mm.avg_citation_precision = (mm.avg_citation_precision / mm.num_valid_run) if mm.num_valid_run > 0 else 0.0
        mm.avg_internal_knowledge_completeness = (mm.avg_internal_knowledge_completeness / mm.num_valid_run) if mm.num_valid_run > 0 else 0.0
        ...
        mm.micro_avg_tool_name_error = (mm.micro_avg_tool_name_error / mm.total_num_tool_call) if mm.total_num_tool_call > 0 else 0.0
        mm.macro_avg_tool_name_error /= mm.num_run

        mm.macro_avg_search_precision /= mm.num_run
        mm.micro_avg_fetch_precision = (mm.micro_avg_fetch_precision / mm.total_num_fetch_call) if mm.total_num_fetch_call > 0 else 0.0
        mm.macro_avg_fetch_precision /= mm.num_run
        mm.macro_avg_raw_search_precision /= mm.num_run
        mm.micro_avg_raw_fetch_precision = (mm.micro_avg_raw_fetch_precision / mm.total_num_fetch_call) if mm.total_num_fetch_call > 0 else 0.0
        ...
        mm.micro_avg_num_correct_search_call = (mm.micro_avg_num_correct_search_call / mm.total_num_search_call) if mm.total_num_search_call > 0 else 0.0
        mm.micro_avg_num_correct_fetch_call = (mm.micro_avg_num_correct_fetch_call / mm.total_num_fetch_call) if mm.total_num_fetch_call > 0 else 0.0
        mm.macro_avg_num_correct_search_call /= mm.num_run
        mm.macro_avg_num_correct_fetch_call /= mm.num_run

        mm.micro_avg_search_params_error = (mm.micro_avg_search_params_error / mm.total_num_search_call) if mm.total_num_search_call > 0 else 0.0
        mm.micro_avg_search_extra_params_error = (mm.micro_avg_search_extra_params_error / mm.total_num_search_call) if mm.total_num_search_call > 0 else 0.0
        mm.micro_avg_fetch_params_error = (mm.micro_avg_fetch_params_error / mm.total_num_fetch_call) if mm.total_num_fetch_call > 0 else 0.0
        mm.micro_avg_fetch_extra_params_error = (mm.micro_avg_fetch_extra_params_error / mm.total_num_fetch_call) if mm.total_num_fetch_call > 0 else 0.0
        mm.micro_avg_fetch_url_error = (mm.micro_avg_fetch_url_error / mm.total_num_fetch_call) if mm.total_num_fetch_call > 0 else 0.0
